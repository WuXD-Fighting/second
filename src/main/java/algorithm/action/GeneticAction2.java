...
//		numList=GetTransitionByXml.getNumSet(tL);
	...
	public void randomSeed()throws Exception {
		//初始化		pathSet = new ArrayList<Path>();
		pathSetList = new ArrayList<List<Path>>();
		List<Transition> tL = new ArrayList<Transition>();
		tL=GetTransitionByXml.getTransitionList();
		reqs = new ArrayList<ReqEnum>();
		reqs = requirementUtil.strForReqs("1,[2,4],3,4,[5,17],[6,16],[7,9,13],[8,15],10,[11,12,14]");
		...
		List<Integer> states = new ArrayList<Integer>();
		//初始生成50条路径
		annealingServiceImpl.getRandomPathList(matrix, 0, 7,  states, temp,5);
		//如果不能完成包含 则新增路径
		while(!setServiceImpl.isCoveredByReqs(reqs, temp)) {
			List<Path> newPath = new ArrayList<Path>();
			annealingServiceImpl.getRandomPathList(matrix, 0, 7,  states, newPath,1);
			temp.addAll(newPath);
		}
...
//		System.out.println("测试总用时:"+set.getSumTime());
	...
	public void randomSeed2()throws Exception {
		...
		while(c++<10) {
			...
			while(be<400) {
				...
				annealingServiceImpl.getRandomPathList(matrix, 0, 21,  states, temp,be+5);
				be = temp.size();
			}
		}
		for(int i = 50 ; i <= 400 ; i ++) {
	...
	public static void main(String[] args) throws Exception{
		GeneticAction2 g = new GeneticAction2();
		g.randomSeed();
