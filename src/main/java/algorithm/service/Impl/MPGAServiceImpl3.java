...
public class MPGAServiceImpl3 implements MPGAService{
	...
	public Set MPGA(MPGAEntity MPGAIndex) {
		...
		while(c++ < 1) {
			...
			population = new ArrayList<List<Set>>();
		...
		while(confirm > 0) {
			...
			for(int i = 0 ; i <population.size() ; i++ ) {
				...
				geneticIndex.setSetList(population.get(i));
				geneticIndex.setShortN(MPGAIndex.getPopulationSize());
				geneticIndex.setMatrix(MPGAIndex.getMatrix());
				geneticIndex.setTimes(40);
...
//						}
	...
	private List<Set> subpopulation(SubpopulationEntity index) throws Exception{
		...
		for( int i = 0 ; i < operateList.size() ; i++) {
			System.out.println("operateList: " + i + "   time " + operateList.get(i).getSumTime());
		}
...
//		}
	...
	private void CHC(List<List<Set>> population , int transferCount) {
		//依次向后迁移 
		for(int i = 0 ; i < population.size() - 1 ; i++ ) {
			List<Set> tempList = new ArrayList<Set>();
			System.out.println("population.get(i):"+population.get(i).size());
		...
		//第一个集合进行更新
		population.set(0, setServiceImpl.selectShortN( population.get(0) ,  20 ));
	}
}
